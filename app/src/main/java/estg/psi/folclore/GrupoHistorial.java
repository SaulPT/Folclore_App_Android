...
public class GrupoHistorial extends Base {
    ...
    protected void onStart() {
        ...
        //TERMINA A ATIVIDADE SE NÃO EXISTIR UM GRUPO SELECIONADO
    ...
    protected void onResume() {
        ...
        if (!verificar_ligacao_internet()) {
            ...
            mostrar_historial(bd.obter_historial(grupo_selecionado));
            bd.close();
        } else {
            //SE SIM, ACEDE À API
            Ion.with(this).load(API_URL + "grupohistorials/" + grupo_selecionado).setTimeout(TIMEOUT).asJsonObject().withResponse().setCallback(new FutureCallback<Response<JsonObject>>() {
                @Override
                public void onCompleted(Exception e, Response<JsonObject> result) {
                    //EM CASO DE ERRO NA LIGAÇÃO
                    if (e != null) {
                        Toast.makeText(GrupoHistorial.this, "Erro na ligação ao servidor", Toast.LENGTH_SHORT).show();
                    } else {
                        ...
                        if (result.getHeaders().code() == 500) {
                            ...
                            Toast.makeText(GrupoHistorial.this, result.getResult().get("message").toString(), Toast.LENGTH_SHORT).show();
                            bd.apagar_historial(grupo_selecionado);
                            if (result.getResult().get("code").getAsInt() == 404) {
                                grupo_selecionado = -1;
                            }
                        } else if (result.getHeaders().code() != 200) {
                            //SE A API DEVOLVEU UM ERRO DESCONHECIDO
                            Toast.makeText(GrupoHistorial.this, "Erro do servidor (" + result.getHeaders().code() + " - " + result.getHeaders().message() + ")", Toast.LENGTH_SHORT).show();
                        } else {
                            //SE A API DEVOLVEU OS DADOS COM SUCESSO, DESERIALIZA E ATUALIZA A BD
                            GsonBuilder gson = new GsonBuilder();
                            gson.setDateFormat(CacheDB.DATE_TIME_FORMAT);
                            Historial historial = gson.create().fromJson(result.getResult(), Historial.class);
                            bd.guardar_historial(historial);
                        }
                    }

                    if (grupo_selecionado != -1) {
                        GrupoHistorial.this.setTitle(bd.obter_grupo(grupo_selecionado).abreviatura);
                        mostrar_historial(bd.obter_historial(grupo_selecionado));
                    } else {
                        finish();
                    }
    ...
    private void mostrar_historial(Historial historial) {
        if (historial == null) {
            textview_historial.setText(null);
            Toast.makeText(this, "O grupo não tem historial", Toast.LENGTH_SHORT).show();
        } else {
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                textview_historial.setText(Html.fromHtml(historial.historial, Html.FROM_HTML_MODE_LEGACY));
            } else {
                textview_historial.setText(Html.fromHtml(historial.historial));
            }
        }

...
public class GrupoHistorial extends Base {
    ...
    public void onResume() {
        ...
        if (!verificar_ligacao_internet()) {
            if (bd.obter_historial(grupo_selecionado) != null) {
                this.setTitle(bd.obter_grupo(grupo_selecionado).abreviatura);
                mostrar_historial(bd.obter_historial(grupo_selecionado).historial);
            } else {
    ...
    private void mostrar_historial(String historial) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            textview_historial.setText(Html.fromHtml(historial, Html.FROM_HTML_MODE_LEGACY));
        } else {
            textview_historial.setText(Html.fromHtml(historial));
        }
    }
